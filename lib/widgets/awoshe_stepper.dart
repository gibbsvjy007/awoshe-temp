...
class AwStep {
  ...
  const AwStep({
    ...
    this.state = AwStepState.indexed,
    this.isActive = false,
  })  : assert(title != null),
        assert(content != null),
        assert(state != null);
...
///  * [Step]
///  * <https://material.google.com/components/steppers.html>
class AwosheStepper extends StatefulWidget {
  ...
  AwosheStepper({
    Key key,
    @required this.steps,
    this.type = AwStepperType.vertical,
    this.currentStep = 0,
    this.onStepTapped,
    this.stepperHeaderBG,
    this.stepperLineColor,
    this.onStepContinue,
    this.onStepCancel,
  })  : assert(steps != null),
        assert(type != null),
  ...
  final List<AwStep> steps;

  /// The type of stepper that determines the layout. In the case of
  /// [AwStepperType.horizontal], the content of the current step is displayed
  /// underneath as opposed to the [AwStepperType.vertical] case where it is
  /// displayed in-between.
  final AwStepperType type;

  final Color stepperHeaderBG;
  final Color stepperLineColor;
...
class _StepperState extends State<AwosheStepper> with TickerProviderStateMixin {
  List<GlobalKey> _keys;
  final Map<int, AwStepState> _oldStates = <int, AwStepState>{};
  ...
  @override
  void didUpdateWidget(AwosheStepper oldWidget) {
  ...
  Widget _buildCircleChild(int index, bool oldState) {
    final AwStepState state = oldState ? _oldStates[index] : widget.steps[index].state;
    final bool isDarkActive = _isDark() && widget.steps[index].isActive;
    assert(state != null);
    switch (state) {
      case AwStepState.indexed:
      case AwStepState.disabled:
        return Text(
          '${index + 1}',
          style: isDarkActive ? _kStepStyle.copyWith(color: Colors.black87) : _kStepStyle,
        );
      case AwStepState.editing:
        return Icon(
          Icons.edit,
          color: isDarkActive ? _kCircleActiveDark : _kCircleActiveLight,
          size: 18.0,
        );
      case AwStepState.complete:
        return Icon(
          Icons.check,
          color: isDarkActive ? _kCircleActiveDark : _kCircleActiveLight,
          size: 18.0,
        );
      case AwStepState.error:
  ...
  Widget _buildCircle(int index, bool oldState) {
    return Container(
      ...
      child: AnimatedContainer(
        ...
        child: Center(
          child: _buildCircleChild(index, oldState && widget.steps[index].state == AwStepState.error),
        ),
      ),
    );
  }
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      return AnimatedCrossFade(
        ...
        crossFadeState:
            widget.steps[index].state == AwStepState.error ? CrossFadeState.showSecond : CrossFadeState.showFirst,
        duration: kThemeAnimationDuration,
      );
    } else {
      if (widget.steps[index].state != AwStepState.error)
  ...
  Widget _buildVerticalControls() {
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          (widget.currentStep < widget.steps.length - 3) ?
          SizedBox(
              width: double.infinity,
              child: FlatButton(
                onPressed: widget.onStepContinue,
                color: _isDark() ? themeData.backgroundColor : primaryColor,
  ...
  Widget _buildVerticalHeader(int index) {
    return Container(
        ...
        child: Row(children: <Widget>[
          ...
          Container(margin: const EdgeInsetsDirectional.only(start: 12.0), child: _buildHeaderText(index))
        ]));
  }
  ...
  Widget _buildVertical() {
    ...
    for (int i = 0; i < widget.steps.length; i += 1) {
      children.add(Column(key: _keys[i], children: <Widget>[
        InkWell(
            onTap: widget.steps[i].state != AwStepState.disabled
                ? () {
            ...
            children: <Widget>[
              Container(
                ...
                child: Column(
                  ...
                  children: <Widget>[_buildHeaderText(i), _buildIcon(i)],
                ),
              ),
//              Container(
//                height: 72.0,
...
//            margin: const EdgeInsets.symmetric(horizontal: 24.0),
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.type) {
      case AwStepperType.vertical:
        return _buildVertical();
      case AwStepperType.horizontal:
